...
class Trillium2026RecruitmentNumberInternalPredictor extends REpiceaPredictor {
	...
	private void setOccupancyInXVector(Trillium2026RecruitmentPlot plot, Species species, double occupancyIndex10km) {
	...
	public synchronized double predictNumberOfRecruits(Trillium2026RecruitmentPlot plot, Species species) {
		...
		if (isUsingOccupancyIndex()) {
			...
			if (isRandomEffectsVariabilityEnabled) {
				double occupancyIndex25kmRandomDeviate = owner.occurrencePredictor.getInternalPredictor(species).getOccupancyRandomDeviate(plot, species);
				setOccupancyInXVector(plot, species, occupancyIndex25kmRandomDeviate);
				return getNumber(beta);
			} else {
	...
	boolean hasOccupancyIndexAmongEffect() {return !occupancyIndexVarIndices.isEmpty();}
