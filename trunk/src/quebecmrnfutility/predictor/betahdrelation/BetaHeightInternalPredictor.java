...
import quebecmrnfutility.predictor.betahdrelation.BetaHeightPredictor.BetaHeightableStandMonteCarlo;
import quebecmrnfutility.predictor.betahdrelation.BetaHeightPredictor.DisturbanceType;
import quebecmrnfutility.predictor.betahdrelation.BetaHeightPredictor.Effect;
...
class BetaHeightInternalPredictor extends HDRelationshipModel<BetaHeightableStand, BetaHeightableTree> {
	...
	protected BetaHeightInternalPredictor(BetaHdSpecies species, boolean isParametersVariabilityEnabled, boolean isRandomEffectsVariabilityEnabled, boolean isResidualVariabilityEnabled) {
	 ...
	 */
	@Override
	protected synchronized RegressionElements fixedEffectsPrediction(BetaHeightableStand stand, BetaHeightableTree t, Matrix beta) {
...
//				break;
	...
	@Override
	protected Collection<BetaHeightableTree> getTreesFromStand(BetaHeightableStand stand) {
		Collection<BetaHeightableTree> treesToBeReturned = new ArrayList<BetaHeightableTree>();
		Collection<?> trees = stand.getTrees();
		if (trees != null && !trees.isEmpty()) {
			for (Object tree : trees) {
				if (tree instanceof BetaHeightableTree) {
					BetaHeightableTree t = (BetaHeightableTree) tree;
